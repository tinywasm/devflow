...
- `SecretStore` unifies credential management by prioritizing `GH_TOKEN`, `GITHUB_TOKEN`, and `JULES_API_KEY` environment variables over the system keyring with thread-safe, lazy initialization.
